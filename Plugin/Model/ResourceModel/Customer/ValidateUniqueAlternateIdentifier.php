 ...
 */
class ValidateUniqueAlternateIdentifier
{
    ...
    public function beforeSave(ResourceModel $subject, Customer $customer)
    {
        ...
        $collection = $this->customerCollectionFactory
            ->create()
            ->addAttributeToFilter(InstallData::ALTERNATIVE_IDENTIFIER, $customer->getData(InstallData::ALTERNATIVE_IDENTIFIER));
        ...
        if ($collection->getSize() > 0) {
            throw new LocalizedException(
                __('A customer with the same identifier already exists in an associated website.')
            );
        }
    }
}
